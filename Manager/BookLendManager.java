...
public class BookLendManager {
	...
	public void lend(String barcode, String readerid) throws BaseException {
		...
		if(r==null) throw new BusinessException("食材不存在");//读者不存在
		if(r.getRemoveDate()!=null) throw new BusinessException("食材已删除");//读者已注销
		if(r.getStopDate()!=null) throw new BusinessException("食材缺货");//读者已挂失
		BeanBook book=(new BookManager()).loadBook(barcode);
		if(book==null) throw new BusinessException("");//图书不存在
		if(!"published".equals(book.getState())) throw new BusinessException("菜谱"+book.getState());
		...
		try {
			...
			pst.execute();
			pst.close();
			sql="update BeanBook set state='已出库' where barcode=?";
	...
	public void returnBook(String barcode) throws BaseException {
		...
		try {
			...
			java.sql.ResultSet rs=pst.executeQuery();
			if(!rs.next()){
				throw new BusinessException("该菜谱没有浏览记录");
			}
			int id=rs.getInt(1);
			...
			pst.execute();
			pst.close();
			sql="update BeanBook set state='published' where barcode=?";
			pst=conn.prepareStatement(sql);
			pst.setString(1,barcode);
			pst.execute();
			conn.commit();
