...
public class ReaderManager {
	...
	public  void createReaderType(BeanReaderType rt) throws BaseException{
		...
		if(rt.getLendBookLimitted()<0 || rt.getLendBookLimitted()>100){
			throw new BusinessException("购买食材数量必须在0-100之间");
		}
		Connection conn=null;
		try {
			conn=DBUtil.getConnection();
			...
			if(rs.next()) throw new BusinessException("食材类别名称已经被占用");
			rs.close();
			pst.close();
	...
	public void modifyReaderType(BeanReaderType rt)throws BaseException{
		...
		if(rt.getReaderTypeName()==null || "".equals(rt.getReaderTypeName()) || rt.getReaderTypeName().length()>20){
			...
			try {
				...
				java.sql.Statement st=conn.createStatement();
				java.sql.ResultSet rs=st.executeQuery(sql);
				if(!rs.next()) throw new BusinessException("食材类别不存在");
				rs.close();
				st.close();
	...
	public void deleteReaderType(int id)throws BaseException{
		if(id<=0){
			...
			try {
				...
				rs.next();
				int n=rs.getInt(1);
				if(n>0) throw new BusinessException("已经有"+n+"个食材是"+readerTypeName+"了，不能删除");
	...
	public  void createReader(BeanReader r) throws BaseException{
		...
		if(r.getReaderid()==null || "".equals(r.getReaderid()) || r.getReaderid().length()>20){
			throw new BusinessException("食材证号必须是1-20个字");
		}
		if(r.getReaderName()==null || "".equals(r.getReaderName()) || r.getReaderName().length()>20){
			throw new BusinessException("食材姓名必须是1-20个字");
		}
		Connection conn=null;
		try {
			conn=DBUtil.getConnection();
			...
			rs=pst.executeQuery();
			if(rs.next()) throw new BusinessException("食材编号已经被占用");
			rs.close();
			pst.close();
	...
	public void renameReader(String id,String name) throws BaseException{
		if(id==null || "".equals(id) || id.length()>20){throw new BusinessException("食材证号必须是1-20个字");}
		if(name==null || "".equals(name) || name.length()>20){throw new BusinessException("食材姓名必须是1-20个字");}
		Connection conn=null;
		try {
			conn=DBUtil.getConnection();
			...
			if(rs.getDate("removeDate")!=null) throw new BusinessException("食材已经注销");
			rs.close();
			pst.close();
	...
	public void changeReaderType(String id,int readerTypeId) throws BaseException{
		if(id==null || "".equals(id) || id.length()>20){throw new BusinessException("食材编码必须是1-20个字");}
		if(readerTypeId<0){throw new BusinessException("食材类别编号不存在");}
		Connection conn=null;
		try {
			conn=DBUtil.getConnection();
			String sql="select * from BeanReader where readerid=?";
			...
			if(!rs.next()) throw new BusinessException("食材不存在");
			if(rs.getDate("removeDate")!=null) throw new BusinessException("食材已经注销");
			if(rs.getInt("readerTypeId")==readerTypeId) throw new BusinessException("没有改变食材类别");
			rs.close();
			pst.close();
	...
	public void stopReader(String id,String doUserId)throws BaseException{
		...
		try {
			...
			if(!rs.next()) throw new BusinessException("食材不存在");
			if(rs.getDate("removeDate")!=null) throw new BusinessException("食材已经注销");
			if(rs.getDate("stopDate")!=null) throw new BusinessException("该食材已经缺货");
			rs.close();
			pst.close();
	...
	public void reuseReader(String id,String doUserId)throws BaseException{
		...
		try {
			...
			if(!rs.next()) throw new BusinessException("食材不存在");
			if(rs.getDate("removeDate")!=null) throw new BusinessException("食材已经注销");
			if(rs.getDate("stopDate")==null) throw new BusinessException("该食材未缺货");
			rs.close();
			pst.close();
